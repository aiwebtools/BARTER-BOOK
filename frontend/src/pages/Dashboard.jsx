...
import { Handshake, ListPlus, Sparkle, MapPinLine, Users, ArrowsClockwise } from "@phosphor-icons/react";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 pb-24 md:pb-8" data-testid="dashboard-page">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="font-heading text-3xl sm:text-4xl font-bold">Welcome back, {user?.display_name?.split(" ")[0]}.</h1>
        ...
        <Button onClick={() => nav("/new")} className="rounded-full h-12 px-6" data-testid="dashboard-post-btn"><ListPlus size={20} weight="bold" className="mr-1" /> Post something</Button>
      </div>

      {/* Stats bento */}
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
          <StatCard n={stats.haves} label="Haves nearby" icon={Sparkle} />
          <StatCard n={stats.needs} label="Needs posted" icon={Handshake} />
          <StatCard n={stats.services} label="Skills offered" icon={Users} />
          <StatCard n={stats.completed_trades} label="Trades completed" icon={ArrowsClockwise} />
        </div>
      )}
